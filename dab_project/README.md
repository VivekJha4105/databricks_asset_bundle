...
- Three environments to work on: DEV, QA and PROD
- Each have their own catalog, making `catalog_name` a dynamic ENVIRONMENT variable.
- We declare it using the `dbutils.widget.text("catalog_name", "")`
- So `catalog_name` parameter will be available for every target ENVIRONMENT to define.
